...
source_image_directory_root = opt.pos4
print('$4=',source_image_directory_root) #hw3_data/digits/mnistm/test
source_domain_name = opt.pos5
print('$5=',source_domain_name) #mnistm, usps or svhn.
source_csv_root = opt.pos6
print('$6=',source_csv_root) #hw3_data/digits/mnistm/test_pred.csv
...
source_dataset = Test_Dataset(img_root=source_image_directory_root, transform=augmentation)
source_dataloader = DataLoader(source_dataset, batch_size=64, shuffle=False) #mnistm=10000 svhn=26032 usps=2007
...
for epoch in range(1):
    with torch.no_grad():
        ...
        img_index_list2 = []
        z_vector2 = torch.zeros(1,3,32,32)
        z_vector2 = z_vector2.cuda()
        for i, (test_data, image_fn) in enumerate(source_dataloader):
            for j in range(len(image_fn)):
                    img_index = (image_fn[j].split('/', -1))[-1]
                    #print(img_index)
                    img_index_list.append(img_index)
            batch_size = test_data.shape[0]
            #print(batch_size)
            z2 = Variable(FloatTensor(np.random.uniform(-1, 1, (batch_size, opt.latent_dim))))
            test_data = test_data.cuda()
            test_data = Variable(test_data.type(FloatTensor).expand(batch_size, 3, opt.img_size, opt.img_size))
            z_vector_batch2 = generator(test_data, z2)
            #print(z_vector_batch2.shape)
            z_vector_batch2 = z_vector_batch2.cuda()
            z_vector2 = torch.cat((z_vector2, z_vector_batch2), 0)
        z_vector2 = z_vector2.view(z_vector2.shape[0], 32*32*3)
        #print(z_vector2.shape)
        z_vector2 = z_vector2[1:, :]
        #print(z_vector.shape)
        z_vector2 = z_vector2.cpu()
        print(z_vector2.shape)
        
        z_vector = z_vector[0:2000, :]
        z_vector2 = z_vector2[0:2000, :]
        total_z_vector = torch.cat((z_vector, z_vector2), 0)
        label_list = []
        for i in range(z_vector.shape[0]):
            label_list.append(0)
        for j in range(z_vector2.shape[0]):
            label_list.append(1)
        
        print(total_z_vector.shape)
        print(len(label_list))
        

        z_tsne = manifold.TSNE(n_components=2, init='random', random_state=5, verbose=1).fit_transform(total_z_vector)
        #Data Visualization
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig("./tsne_output_svhn_usps.png")
        plt.show()
