...
print('$3=',testing_csv_root)

source_image_directory_root = sys.argv[4]
print('$4=',source_image_directory_root) #hw3_data/digits/mnistm/test
source_domain_name = sys.argv[5]
print('$5=',source_domain_name) #mnistm, usps or svhn.
source_csv_root = sys.argv[6]
print('$6=',source_csv_root)
...
source_dataloader = DataLoader(source_dataset, batch_size=128, shuffle=False) #mnistm=10000 svhn=26032 usps=2007
    ...
    with torch.no_grad():  
            ...
            for i, (test_data, image_fn) in enumerate(source_dataloader):
                ...
                for j in range(len(image_fn)):
                    img_index = (image_fn[j].split('/', -1))[-1]
                    #print(img_index)
                    img_index_list2.append(img_index)

                test_data = test_data.cuda()
                z_vector_batch = (feature_extractor(test_data))
                z_vector_batch = z_vector_batch.cuda()
                #print(z_vector_batch.shape)
                z_vector2 = torch.cat((z_vector2, z_vector_batch), 0)
                #print(z_vector.shape)
            ...
            plt.xticks([])
            plt.yticks([])
            plt.savefig("./tsne_output_mnistm_svhn.png")
            plt.show()
